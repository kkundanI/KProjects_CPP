...
//Last float
 ...
 cout<<endl;
  ...
  string line;
    ifstream ffin;
    //OPening Calculation File 
	 ffin.open("CalCulation.txt");
     //Counting numbers of saved Calculation 
    ...
    //File saving Calculation !
	 fout.open("Calculation.txt", ios::app);
	 
     fout<<"Cal "<<count<<": "<<inp<<Num[0]<<endl; 
	int flag;
	cout<<"\n To Print History Enter (1) , else Enter (0) : \n\n";
    cin>>flag;
	fout.close();
	
    if(flag==1){
    ifstream fin;
	 fin.open("Calculation.txt");
...
//Calculation more than one line : Problem in numbering of calculation while saving! /*File Error0*/